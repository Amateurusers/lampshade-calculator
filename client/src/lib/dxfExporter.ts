...
function generateWaveformDXF(result: WaveformLampshadeResult): string {
  ...
  // User's "上口有波浪" (top/smaller opening has waves) → topWave → but we swap it to outerR → appears at top in DXF
  if (topWave) {
  ...
  // User's "下口有波浪" (bottom/larger opening has waves) → bottomWave → but we swap it to innerR → appears at bottom in DXF
  if (bottomWave) {
